...
SELECT EMPLOYEE_ID, FIRST_NAME || ' ' || LAST_NAME AS NAME, SALARY, ROUND(SALARY *1.123) AS "Increase Salary"
    ...
    ORDER BY EMPLOYEE_ID, NAME, SALARY DESC, "Increase Salary" DESC
;
...
*/
SELECT E.JOB_ID, E.AVG_SALARY
FROM (SELECT JOB_ID, AVG(SALARY) AVG_SALARY
FROM EMPLOYEES GROUP BY JOB_ID) E
WHERE E.AVG_SALARY > 10000
;
SELECT E.JOB_ID, E.AVG_SALARY
FROM (SELECT JOB_ID, AVG(SALARY) AVG_SALARY
FROM EMPLOYEES GROUP BY JOB_ID) E
